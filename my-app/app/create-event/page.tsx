  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-black via-[#071130] to-[#021428] text-white px-6 py-12">
      <div className="max-w-5xl mx-auto text-center animate-fade-in">
        {/* Header */}
        <h1 className="text-4xl font-extrabold mb-2 tracking-tight">Create a New Event</h1>
        <div className="w-16 h-1 bg-cyan-500 mx-auto mb-2 rounded-full" />
        <p className="text-gray-400 text-lg mb-10">Share event details so others can join.</p>
        ...
        <div className="grid gap-8 md:grid-cols-2 items-start">
          ...
          <div className="bg-white/5 backdrop-blur-md rounded-2xl p-8 border border-gray-700 shadow-2xl text-left">
            <div className="flex items-center justify-between mb-6">
              <div className="text-xs text-gray-400 bg-gray-900/40 px-3 py-1 rounded-full">Logged in</div>
              <div className="text-sm text-gray-300 font-medium">{user.email ?? user?.user_metadata?.full_name ?? "Organizer"}</div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                <span className={`text-xs ${descriptionCharCount >= 300 ? "text-red-400" : "text-gray-400"}`}>
              ...
              {descriptionCharCount >= 300 && (
                <p className="text-xs text-red-400 mt-1">Character limit reached. You cannot add more characters.</p>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs text-gray-300 font-medium">Date</label>
                <div className="relative mt-2">
                  <input
                    type="date"
                    name="date"
              ...
              <div>
                <label className="text-xs text-gray-300 font-medium">Time</label>
                <div className="relative mt-2">
                  <input
                    type="time"
                    name="time"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-xs text-gray-300 font-medium">Duration (min)</label>
                <input
                  type="number"
                  name="time_length"
            ...
            <div className="grid grid-cols-2 gap-4 items-end">
              <div>
                <label className="text-xs text-gray-300 font-medium">Tags</label>
                <div className="mt-2">
                  <Tags 
                    tags={form.tags}
            ...
            <div className="mt-6 bg-gradient-to-br from-gray-900/50 to-gray-900/30 rounded-xl p-5 border border-gray-700 overflow-hidden">
              <div className="flex items-start gap-4">
                <div className="w-14 h-14 rounded-lg bg-cyan-400/10 flex items-center justify-center border border-cyan-600 flex-shrink-0">
                  <div className="text-cyan-300 font-bold text-xl">{form.title ? form.title.charAt(0).toUpperCase() : "E"}</div>
                </div>

                <div className="flex-1 min-w-0">
                  <h4 className="text-xl font-semibold text-white leading-tight break-words">{form.title || "Untitled Event"}</h4>
                  <p className="text-sm text-gray-300 mt-1 break-words">{form.organizer || "Hosted by Organizer"}</p>

                  <div className="mt-3 flex flex-col gap-2 text-sm text-gray-300">
                    <div className="flex items-start gap-2">
                      <Calendar className="text-cyan-400 flex-shrink-0 mt-0.5" size={16} />
                    ...
                    <div className="flex items-start gap-2">
                      <MapPin className="text-cyan-400 flex-shrink-0 mt-0.5" size={16} />
                      <div className="break-words">{form.location || "Location"}</div>
                    </div>
                  </div>

                  <div className="mt-4">
                    <p className="text-sm text-gray-400 italic break-words">[Description would appear here]</p>
                  </div>
                  ...
                  <div className="mt-4 flex items-center gap-3">
                    ...
                    <div className="text-sm text-gray-300">{form.capacity || "—"}</div>
                  </div>

                  <div className="mt-4 flex flex-wrap gap-2">
                    {tagArray.length > 0 ? (
                      tagArray.map((t) => (
                        <span key={t} className="text-xs px-2 py-1 rounded-full bg-cyan-400/10 border border-cyan-600 text-cyan-100">#{t}</span>
                      ))
                    ) : (
                      <span className="text-xs px-2 py-1 rounded-full bg-gray-900/40 text-gray-400">No tags yet</span>
                    )}
                  </div>
                </div>
              </div>
